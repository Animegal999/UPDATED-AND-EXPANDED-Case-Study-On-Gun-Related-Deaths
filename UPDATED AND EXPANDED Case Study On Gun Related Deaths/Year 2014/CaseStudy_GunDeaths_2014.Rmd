...
Author: Amber P. Giza
File Format and Title: CaseStudy_GunDeaths_2014.Rmd
---
...
and data, which shows us 33,599 total rows. 
...
gunDeathsData_2014 <- read.csv("/content/guns_2014.csv", header = TRUE, sep = ",")
View(gunDeathsData_2014)
```
...
print("Mean Age: ") # 44.228
mean(gunDeathsData_2014$age)
```
...
print("Standard Deviation of the age:") #19.699 years
SD(gunDeathsData_2014$age)
```
...
* Asian/Pacific Islander victims and White/Caucasian victims had the most "normally" distributed data for age

```{r, echo=FALSE}
# creating boxplot on age and race:
boxplot(gunDeathsData_2014$age ~ gunDeathsData_2014$race, data = gunDeathsData_2014,
...
hist(gunDeathsData_2014$age, xlab="Victims of Gun Violence By Age", col = "red", border = "black")
```
...
boxplot(gunDeathsData_2014$age ~ gunDeathsData_2014$sex, data = gunDeathsData_2014,
        main = "Age of Victims of Gun-related Deaths by Gender",
        xlab = "Gender",
        ylab = "Age")
```

This next histogram shows the number of deaths by month. This also shows that August had the most 
victims.

```{r, echo=FALSE}
# Constructing the histogram for number of deaths by month:

hist(gunDeathsData_2014$month, xlab="Total Victims of Gun Violence By Month", col = "blue", border = "black")
```
...
* The Accidental deaths, Undetermined Intentions, and Suicide boxplots have the most "normally" distributed data

```{r, echo=FALSE}
# Creating a boxplot on age and intentions behind the gun:
boxplot(gunDeathsData_2014$age ~ gunDeathsData_2014$intent, data = gunDeathsData_2014,
...
```{r}
gunDeathsData_2014 %>%
  count(gunDeathsData_2014$race)
```

## For the percentages: (Total is 33,599 victims)
* Asian/Pacific Islander -> 1.24%
* African American/Black -> 22.73%
* Hispanic -> 8.56%
...
```{r}
gunDeathsData_2014 %>%
  count(gunDeathsData_2014$intent)
```

## For the percentages:
* Accidental Deaths -> 1.74%
* Homicides -> 33.96%
* Suicides -> 63.496%
* Undetermined -> 0.804%
...
```{r}
gunDeathsData_2014 %>%
  count(gunDeathsData_2014$month)
```
